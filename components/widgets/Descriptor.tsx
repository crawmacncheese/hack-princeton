    ...
    <div className={`${className} flex`}>
      {emotions.length > 0 && (
        <div className="mb-3 flex rounded-full border border-neutral-200 text-sm shadow">
          <div className="flex justify-center rounded-l-full bg-white px-3 py-2 font-medium text-neutral-800"></div>
          <div className="w-48 bg-neutral-800 px-4 py-2 text-center lowercase text-white">
            <span>{createDescription(stableEmotions)}</span>
          </div>
          <div className="flex justify-center rounded-r-full bg-white px-3 py-2 font-medium text-neutral-800"></div>
        </div>
      )}
    </div>
  );
